...
def test_guard_blocks_behavior_change_without_tests() -> None:
    result = evaluate_changed_files(["annuity_model/src/annuity_model/pricing_ui.py"])
    assert not result.ok
    assert result.behavior_files == ("annuity_model/src/annuity_model/pricing_ui.py",)
...
def test_guard_passes_when_behavior_and_tests_change_together() -> None:
    result = evaluate_changed_files(
        [
            "annuity_model/src/annuity_model/portfolio_runner.py",
            "annuity_model/tests/parity/test_portfolio_golden.py",
        ]
    )
    assert result.ok
    assert result.behavior_files == ("annuity_model/src/annuity_model/portfolio_runner.py",)
