...
def test_vul_collapses_to_ul_with_zero_subaccount_return():
    ...
    res_vul = vul.price_vul_single_premium(
        contract=contract_vul, yield_curve=_yc(), mortality=_mort(),
        horizon_age=80, expenses=sp.ExpenseAssumptions(0.0, 0.0, 0.0),
        index_s0=100.0, index_levels_payment=flat_levels,
    )
    res_ul = ul.price_ul_single_premium(
        contract=contract_ul, yield_curve=_yc(), mortality=_mort(),
        horizon_age=80, expenses=sp.ExpenseAssumptions(0.0, 0.0, 0.0),
    )
...
def test_vul_mc_mean_within_band():
    ...
    res = vul.price_vul_single_premium_monte_carlo(
        contract=contract, yield_curve=_yc(), mortality=_mort(),
        horizon_age=80, expenses=sp.ExpenseAssumptions(0.0, 0.0, 0.0),
        n_sims=40, annual_drift=0.06, annual_vol=0.15, seed=42, s0=100.0,
    )
...
def test_vul_drift_increase_raises_av():
    ...
    base = vul.price_vul_single_premium_monte_carlo(
        contract=contract, yield_curve=_yc(), mortality=_mort(),
        horizon_age=80, expenses=sp.ExpenseAssumptions(0.0, 0.0, 0.0),
        n_sims=20, annual_drift=0.04, annual_vol=0.15, seed=42, s0=100.0,
    )
    higher = vul.price_vul_single_premium_monte_carlo(
        contract=contract, yield_curve=_yc(), mortality=_mort(),
        horizon_age=80, expenses=sp.ExpenseAssumptions(0.0, 0.0, 0.0),
        n_sims=20, annual_drift=0.10, annual_vol=0.15, seed=42, s0=100.0,
    )
