...
def test_va_actuarial_sanity_signs():
    ...
    res = va.price_va_single_premium(
        ...
        horizon_age=75, expenses=sp.ExpenseAssumptions(0.0, 0.0, 0.0),
...
def test_va_collapses_when_me_zero_and_flat():
    ...
    contract = va.VAContract(
        issue_age=55, sex="male", single_premium=100_000.0,
        me_charge_annual=0.0, horizon_years=10,
    )
    n = 120
    levels = np.full(n, 100.0)
    res = va.price_va_single_premium(
        contract=contract, yield_curve=_yc(), mortality=_mort(),
        horizon_age=65, expenses=sp.ExpenseAssumptions(0.0, 0.0, 0.0),
        index_s0=100.0, index_levels_payment=levels,
    )
    av_t = float(res.account_value_end_month[-1])
...
def test_va_mc_mean_within_band():
    ...
    res = va.price_va_single_premium_monte_carlo(
        contract=contract, yield_curve=_yc(), mortality=_mort(),
        horizon_age=75, expenses=sp.ExpenseAssumptions(0.0, 0.0, 0.0),
        n_sims=80, annual_drift=0.06, annual_vol=0.15, seed=42, s0=100.0,
    )
...
def test_va_drift_increase_raises_av():
    ...
    higher = va.price_va_single_premium_monte_carlo(
        contract=contract, yield_curve=_yc(), mortality=_mort(),
        horizon_age=75, expenses=sp.ExpenseAssumptions(0.0, 0.0, 0.0),
        n_sims=40, annual_drift=0.10, annual_vol=0.15, seed=42, s0=100.0,
    )
