...
def build_fia_workbook_from_spec(
    ...
    *,
    ...
    write_inputs_sheet(ws_in, InputsSheetSpec(title="FIA Inputs (matches model launcher / Python)", rows=rows))
    nm = (
    ...
    for r in range(first, last_cap_row + 1):
        ...
        ws_pr.cell(row=r, column=7, value=f'=IF({a}="","",IFERROR(INDEX({idx_b},MATCH({a},{idx_a},0)),""))')
        ws_pr.cell(
            row=r, column=8,
            value=(
        ...
        ws_pr.cell(row=r, column=10, value=f'=IF({a}="","",IF({a}=1,{sp_ref}*(1+I{r}),J{r-1}*(1+I{r})))')
    ...
    write_model_check_sheet(
        wb, snap_py, alm_layout=None, alm_snapshot=None,
