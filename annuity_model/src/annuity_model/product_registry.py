...
from annuity_model.build_fia_excel_workbook import FIAExcelBuildSpec, fia_excel_spec_from_launcher
from annuity_model.build_iul_excel_workbook import IULExcelBuildSpec, iul_excel_spec_from_launcher
from annuity_model.build_myga_excel_workbook import (
    MYGAExcelBuildSpec,
    myga_excel_spec_from_launcher,
)
from annuity_model.build_pricing_excel_workbook import ExcelBuildSpec, excel_spec_from_launcher
...
from annuity_model.build_ul_excel_workbook import ULExcelBuildSpec, ul_excel_spec_from_launcher
from annuity_model.build_va_excel_workbook import VAExcelBuildSpec, va_excel_spec_from_launcher
from annuity_model.build_vul_excel_workbook import VULExcelBuildSpec, vul_excel_spec_from_launcher
from annuity_model.build_wl_excel_workbook import WLExcelBuildSpec, wl_excel_spec_from_launcher
...
def validate_run_inputs(product_type: ProductType, state: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    P1 (2026-04) introduced this hook as a *single source of truth* for
    "is this run launchable" so the validation does not get scattered
    across pricing_ui.py with stringly-typed conditionals. Chunk 4 moved
    per-product validator selection into ``ProductDefinition``; the private
    ``_PRODUCT_VALIDATORS`` table remains only as a seed for definitions.
    ...
    * ``horizon_age > issue_age`` (otherwise the projection is empty).
    ...
    Per-product extensions are selected from
    :func:`annuity_model.products.product_validators_by_type`.
    """
    errors: list[str] = []
    ...
    validator = product_validators_by_type().get(product_type)
...
def get_product_adapter(product_type: ProductType) -> ProductAdapter:
    """Return the canonical adapter for *product_type*.

    The public adapter lookup is derived from ``ProductDefinition``. The
    private ``_PRODUCT_ADAPTERS`` dict remains only as the construction seed
    used by product shims during import.
    """
    from annuity_model.products import get_product_definition

    try:
        return cast(ProductAdapter, get_product_definition(product_type).adapter)
    except KeyError as exc:
        product_name = getattr(product_type, "value", repr(product_type))
        raise NotImplementedError(
            f"Pricing adapter is not implemented for product {product_name!r}."
        ) from exc


def product_adapters_by_type() -> Mapping[ProductType, ProductAdapter]:
    """Read-only adapter compatibility view derived from ProductDefinition."""
    from annuity_model.products import product_adapters_by_type as canonical_view
...
def implemented_product_types() -> tuple[ProductType, ...]:
    ...
    Used by meta-tests to assert every implemented product also has an entry
    in ``LIABILITY_LAYOUTS``.
    """
...
def product_options_for_ui() -> list[ProductType]:
    from annuity_model.products import product_options_for_ui as canonical_options

    return list(canonical_options())


def product_label(product_type: ProductType) -> str:
    from annuity_model.products import get_product_definition

    return get_product_definition(product_type).display_name


def get_product_capabilities(product_type: ProductType) -> ProductCapabilities:
    from annuity_model.products import get_product_definition

    return cast(ProductCapabilities, get_product_definition(product_type).capabilities)


def get_product_mortality_mode_options(product_type: ProductType) -> tuple[str, ...]:
    from annuity_model.products import get_product_definition

    return get_product_definition(product_type).mortality_mode_options


def get_product_default_mortality_mode(product_type: ProductType) -> str:
    from annuity_model.products import get_product_definition

    return get_product_definition(product_type).default_mortality_mode
...
def get_pricing_metrics(product_type: ProductType, result: Any) -> tuple[PricingMetric, ...]:
    ...
    No silent fallback: if a product definition is missing a formatter, raise
    a ``KeyError`` instead of silently returning SPIA-shaped metrics that
    would mislead the UI.
    """
    from annuity_model.products import pricing_metric_formatters_by_type as canonical_view

    formatter = canonical_view().get(product_type)
    if formatter is None:
        raise KeyError(
            f"No pricing-metric formatter registered for {product_type.value!r}; "
            f"add it to the product's ProductDefinition."
        )
    return cast(tuple[PricingMetric, ...], formatter(result))


def pricing_metric_formatters_by_type() -> Mapping[
    ProductType, Callable[[Any], tuple[PricingMetric, ...]]
]:
    """Read-only metric-formatter compatibility view derived from definitions."""
    from annuity_model.products import pricing_metric_formatters_by_type as canonical_view
...
def get_product_ui_config(product_type: ProductType) -> ProductUIConfig:
    ...
    return cast(ProductUIConfig, get_product_definition(product_type).ui_config)
