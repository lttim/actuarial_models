"""WL (Whole Life — single premium) Excel workbook builder.

Per-month death-cashflow grid; ``total_cf_col=S, discount_col=O`` so the
ALM helper finds the SPIA/Term-style layout.
"""
...
SHEET_INPUTS = "Inputs"
SHEET_QX = "QxTable"
WL_PROJ_MAX_ROWS = 1000  # WL goes to age 120, so longer horizon
...
def wl_excel_spec_from_launcher(
    *,
    contract: wl.WLContract,
    yield_curve: sp.YieldCurve,
    mortality,
...
def build_wl_workbook_from_spec(
    spec: WLExcelBuildSpec,
    *,
    ...
    if isinstance(mort_for_qx, MortalityTable2017CSO):
        ...
        # The Excel QxTable sheet expects a flat (age, qx) table. RP+MP
        # is a generational table; collapse to a single calendar-year
        # snapshot using the spec.valuation_year so the Excel sheet has
        # something concrete to reference. Production users wanting the
        # generational behavior should view the Python-literal cells.
    ...
    write_monthly_curve_logdf(ws_mc_curve, n_months=WL_PROJ_MAX_ROWS, y_last_row=y_last_row)
    ...
    last_cap_row = 3 + WL_PROJ_MAX_ROWS
    first = 4
    ...
    ws_pr["A1"] = "Whole Life liability cashflows & pricing (formula-driven)"
    ...
    for r in range(first, last_cap_row + 1):
        ...
        ws_pr.cell(row=r, column=7, value=f'=IF({a}="",0,{face_ref}*F{r})')
    ...
    wl_rows = [
        ...
        ("PV net (claims + expenses)", pv_t, f"={LIABILITY_SHEET_NAME}!X7", "money"),
    ...
    write_model_check_sheet(
        ...
        subtitle=(
            ...
            "Mortality from QxTable sheet (CSO 2017 Ultimate placeholder by default)."
        ),
    )
