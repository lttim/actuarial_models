"""FIA (Fixed Indexed Annuity) pricing engine.

Single-premium deferred annuity with crediting linked to an index.
Floor=0 (typical FIA, no negative crediting); cap and participation
applied to the annual point-to-point return on segment anniversaries.

Cashflow shape (Phase 2, Section 3 of ``docs/seven_product_rollout_plan.md``):
...
``supports_monte_carlo=True`` -- reuses the GBM index simulator from
:mod:`pricing_projection`.
"""
...
from annuity_model.crediting import AnnualPointToPointCapped
...
def _fia_av_path(
    *,
    contract: FIAContract,
    L: np.ndarray,
    n_months: int,
) -> tuple[np.ndarray, np.ndarray]:
    """Walk the AV path month by month; return (av_end, segment_credit_per_month)."""
    seg = int(contract.segment_months)
    if seg != 12:
        raise ValueError("FIA v1 only supports segment_months=12.")
    ...
    for m in range(1, n_months + 1):
        ...
        av *= 1.0 - fee_m
        av_end[m - 1] = float(av)
    return av_end, cred_m


@traced("pricing.fia.deterministic")
def price_fia_single_premium(
    *,
    contract: FIAContract,
    ...
    if contract.payment_freq_per_year != 12:
        raise ValueError("FIA scaffold assumes monthly frequency.")
    ...
    av_end, cred_m = _fia_av_path(contract=contract, L=L, n_months=n_months)
...
@traced("pricing.fia.monte_carlo")
def price_fia_single_premium_monte_carlo(
    *,
    contract: FIAContract,
    yield_curve: sp.YieldCurve,
    mortality: sp.MortalityTableQx | sp.MortalityTableRP2014MP2016,
...
def liability_path_from_fia_projection(pricing: FIAProjectionResult) -> sp.LiabilityPath:
