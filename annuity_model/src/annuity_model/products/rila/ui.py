...
from annuity_model.product_registry import (
    _RILA_ADAPTER,
    ProductUIConfig,
    _rila_pricing_metrics,
)

RILA_ADAPTER = _RILA_ADAPTER
rila_metric_formatter = _rila_pricing_metrics
rila_ui_config = ProductUIConfig(
...
if TYPE_CHECKING:
    from annuity_model.rila_projection import RILAContract
...
def build_rila_contract_from_session(
    ...
    *,
    ...
    death_benefit_type = cast(
        Literal["account_value", "return_of_premium"],
        str(session_state.get(RUN_KEY.RILA_DEATH_BENEFIT_TYPE, "account_value")),
    )
