...
| RILA accumulation (`rila_projection.py`, `build_rila_excel_workbook.py`) | 1 | Product actuary | Independent actuarial reviewer | RILA accumulation pricing and model-check replication | Per PR + release | v1 excludes GLWB/ratchets and advanced rider behavior. |
| Life accumulation products (`wl_projection.py`, `ul_projection.py`, `iul_projection.py`, `vul_projection.py`) | 1 | Product actuary | Independent actuarial reviewer | Life-product premium/account-value projections | Per PR + release | Synthetic CSO placeholder risk if not overlaid with licensed data. |
